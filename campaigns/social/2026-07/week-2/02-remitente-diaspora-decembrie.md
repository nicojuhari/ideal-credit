---
name: Remitențe diaspora - luna record
type: social-post
created: 2026-07-08
updated: 2026-07-11
lane: Global finance & money numbers (Moldova-anchored)
format: single-image (short post)
platforms: Facebook, LinkedIn, Instagram (same caption everywhere)
status: draft
---

# Post - Luna în care diaspora trimite cei mai mulți bani acasă

## Quality gate check

- On-topic rule: yes - a money/finance number (remittance flows), closed on a real credit-
  relevant takeaway (timing gap between when money is needed and when it arrives).
- Full-picture rule: yes - dă și cifra lunii record, și media lunară, ca cititorul să vadă
  diferența reală, nu doar un număr izolat.
- Send-to-someone test: cineva cu părinți/frați plecați la muncă îl trimite unei rude din
  diasporă - „uite, se vede și în statistici ce faceți voi de sărbători."
- Sourced: date BNM privind intrările valutare de la persoane fizice, ianuarie-decembrie 2025.
  No invented figures - media lunară e un calcul simplu (total/12), arătat transparent.
- Language check: un singur termen ceva mai tehnic („intrări valutare" / „remitențe"),
  explicat direct în text. Restul, propoziții scurte, ușor de urmărit de orice vârstă.
- Sign-off present: yes.

---

## The image (single frame)

**173 milioane $**

luna cu cei mai mulți bani trimiși acasă de moldovenii din străinătate

_(jos, mic:)_ Sursă: BNM, 2025

_(Hook + o singură linie de deschidere. Fără linia bold cu Decembrie/26%/138 milioane -
detaliul ăsta se mută integral în text, per regula din [[social-design-system]].)_

---

## Caption (single version, LinkedIn register - used on all platforms)

173 milioane de dolari - atât au trimis acasă moldovenii plecați la muncă în străinătate, într-o
singură lună din 2025.

Luna? Decembrie. Cu aproape 26% peste media lunară a anului (138 milioane $). Nu e întâmplător

- e luna sărbătorilor, a cadourilor mari și, pentru multe familii, a reparațiilor sau
  cumpărăturilor amânate peste an.

Partea practică: banii diasporei nu ajung mereu exact când e nevoie de ei - un cadou de Crăciun,
o urgență medicală sau o renovare pot apărea înainte ca transferul să ajungă. Pentru acele
momente, un credit pe termen scurt acoperă diferența de timp, nu de bani - se achită ușor imediat
ce suma promisă ajunge.

Ai simțit și tu diferența asta în familie, în decembrie?

Sursă: Banca Națională a Moldovei.

Ideal Credit - credite pentru succes!

#IdealCredit #CreditMoldova #DiasporaMoldova #CreditPersonal

---

## Design notes for whoever builds the image

- Big number dominates the frame, same visual weight/position as prior posts (consistency
  across the page)
- One short context line under the hook number - nothing else. No secondary bolded stat, no
  December/26%/138 million detail in-frame; that's caption content now.
- Source line small, bottom, same spot every post - recognizable brand element
- In-frame stays pure content (no CTA graphic) - the brand sign-off lives in the caption text
