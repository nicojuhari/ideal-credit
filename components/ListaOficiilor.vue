...
<template>
    <div class="grid grid-cols-1 gap-10">
        <div v-for="item in list">
            <div class="text-lg underline underline-offset-2 mb-2">{{ item.title }}</div>
            <div>{{ item.address }}</div>
            <div>{{ item.addressNumbers }}</div>
            <div>Luni - Vineri: 08:30 - 16:30</div>
            <div v-if="item.id == 1" class="text-sm italic mt-2">(este nevoie de programare prealabilă)</div>
            <iframe :src="item.map" width="100%" height="450" class="mt-4" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
        </div>
    </div>
</template>