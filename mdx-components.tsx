...
export function useMDXComponents(components: MDXComponents): MDXComponents {
    return {
        ...
        p: ({ children }) => (
            <p className="leading-relaxed text-gray-400 font-light mb-4">{children}</p>
        ),
        ul: ({ children }) => (
            <ul className="list-disc pl-5 space-y-2 mb-6 text-gray-400 font-light">{children}</ul>
        ),
        ol: ({ children }) => (
            <ol className="list-decimal pl-5 space-y-2 mb-6 text-gray-400 font-light">{children}</ol>
        ),
        ...
        th: ({ children }) => (
            <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 bg-white/3 first:rounded-tl-lg last:rounded-tr-lg">
                {children}
            </th>
        ),
        td: ({ children }) => (
        ...
        blockquote: ({ children }) => (
            <blockquote className="border-l-2 border-brand-500/60 pl-5 my-6 text-gray-400 italic">{children}</blockquote>
        ),
                ...
                <table className="w-full border-collapse text-sm">
                    <thead>
                        <tr className="border-b border-white/8">
                            {headers.map((h, i) => (
                                <th key={i} className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 bg-white/3">
                                    {h}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody>
                        {rows.map((row, ri) => (
                            <tr key={ri} className={`border-b border-white/5 last:border-0 ${highlight?.includes(ri) ? "bg-brand-500/5" : ""}`}>
                                {row.map((cell, ci) => (
                                    <td key={ci} className={`px-4 py-3 font-light ${highlight?.includes(ri) ? "text-brand-500 font-medium" : "text-gray-400"}`}>
                                        {cell}
                                    </td>
                                ))}
                            </tr>
        ...
        InfoBox: ({ children }: { children: React.ReactNode }) => (
            <div className="flex gap-3 rounded-xl border border-white/10 bg-white/3 px-5 py-4 my-6 text-sm text-gray-400">
