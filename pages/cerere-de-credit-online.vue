        ...
        <div class="card light">
            ...
            <FormKit id="cerere-online-form" type="form" method="POST" :actions="false" @submit="submitForm"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <FormKit type="number" step="1" min="6" max="60" name="termen" placeholder="12" label="Termen (luni)"
                        validation="required|number|max:60|min:6" input-class="bg-white bg-opacity-10" :validation-messages="{
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <FormKit type="text" name="prenume" placeholder="Ion" label="Prenume" validation="required|length:3,25"
                        input-class="bg-white bg-opacity-10" :validation-messages="{
                            required: 'Prenumele este obligatoriu',
                            length: 'Cel puțin 3 caractere, maximum 25',
                        }" />
                    <FormKit type="text" name="nume" placeholder="Popescu" label="Nume" validation="required|length:3,25"
                        input-class="bg-white bg-opacity-10" :validation-messages="{
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mt-4">
                    <FormKit type="text" name="adresa_domiciliu" placeholder="Oraș/Sat, Strada, număr, bloc, ap."
                        ...
                        :validation-messages="{
                            ...
                            " />
                    <FormKit type="tel" name="telefon" placeholder="012345678" label="Telefon/Mobil" validation="required"
                        input-class="bg-white bg-opacity-10" :validation-messages="{
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <FormKit type="number" min="0" name="datorii" placeholder="1200" label="Datorii achitate lunare (MDL)"
                        validation="required" help="Dacă sunt alte credite, pune 0 dacă nu ai datorii"
                        input-class="bg-white bg-opacity-10" :validation-messages="{
            ...
            <UiModal @close="formSuccess = false" v-if="formSuccess">
                <p class="text-lg text-white">Cererea de credit a fost trimisă cu succes. Vă vom contacta în cel mai scurt
                    timp posibil.</p>
            </UiModal>
        </div>
    </div>
